...
int tensor_multCorrect_works() {
    ...
    int result_var_arr[4] = { 5, 11, 11, 25 };

    tensor* t_result_mult = tensor_mult(t1, t2);

    if (t_result_mult == NULL) {
        vector_destroy(val);
        vector_destroy(shape);
        tensor_destroy(t1);
        tensor_destroy(t2);
        tensor_destroy(t_result_mult);
        return 0;
    }

    int result = (t_result_mult->data != NULL) && \
        (t_result_mult->shape != NULL) && \
        (t_result_mult->strides != NULL) && \
        (t_result_mult->type == INT) && \
        ((int)t_result_mult->data->size == 4);
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            ...
            int result_val = vint(tensor_get(t_result_mult, idx));
            int expected_val = result_var_arr[i * 2 + j];
...
int test() {
    ...
    RUN_TEST(tensor_multCorrect_works);
